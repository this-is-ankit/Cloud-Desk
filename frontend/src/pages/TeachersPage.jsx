          ...
          <div className="mt-8 grid gap-4 md:grid-cols-3">
            <label className="form-control md:col-span-1">
              ...
              <div className="relative">
                ...
                <input name="q" value={filters.q} onChange={handleChange} className="input input-bordered rounded-2xl w-full pl-11" placeholder="Teacher or expertise" />
              </div>
            </label>
            <label className="form-control">
              <span className="mb-2 text-sm font-medium">Subject</span>
              <input name="subject" value={filters.subject} onChange={handleChange} className="input input-bordered rounded-2xl" placeholder="DSA" />
            </label>
            <label className="form-control">
              <span className="mb-2 text-sm font-medium">Language</span>
              <input name="language" value={filters.language} onChange={handleChange} className="input input-bordered rounded-2xl" placeholder="English" />
            </label>
          </div>
        </div>
                ...
                <article key={teacher._id} className="rounded-[2rem] border border-base-content/10 bg-base-100 p-6">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-base-content/60">{teacher.headline || "Live teacher"}</p>
                    </div>
                  </div>
                  <p className="mt-4 text-sm leading-6 text-base-content/70">{teacher.bio || "This teacher is available for live classes on Cloud Desk."}</p>
                  ...
                  <div className="mt-5 text-sm text-base-content/65">
                    <p>Published courses: {teacher.stats?.publishedCourseCount || 0}</p>
                    <p>Upcoming classes: {teacher.stats?.upcomingClassCount || 0}</p>
                    <p>Availability: {teacher.availabilityNote || "Shared on profile"}</p>
                  </div>
                  <button className="btn btn-primary btn-sm rounded-xl mt-5" onClick={() => navigate(`/teachers/${teacher._id}`)}>
                    View teacher
                  </button>
                </article>
              ))}
