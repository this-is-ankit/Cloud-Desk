    ...
    <div className="min-h-screen bg-base-100 flex flex-col font-jakarta">
      ...
      <PageContainer className="flex-grow">
        ...
        <div className="mb-10 pt-4">
          <h1 className="text-3xl md:text-4xl font-bold mb-3 tracking-tight text-base-content">Practice Problems</h1>
          <p className="text-lg text-base-content/60 font-medium max-w-2xl">
            Sharpen your coding skills with these curated exercises. Master the fundamentals before joining a live cohort.
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
          <div className="bg-base-100 border border-base-200 p-5 rounded-2xl shadow-sm flex flex-col">
            <span className="text-sm font-semibold text-base-content/60">Total Problems</span>
            <span className="text-3xl font-bold mt-1 text-base-content">{problems.length}</span>
          </div>
          <div className="bg-base-100 border border-base-200 p-5 rounded-2xl shadow-sm flex flex-col">
            <span className="text-sm font-semibold text-success">Easy</span>
            <span className="text-3xl font-bold mt-1 text-base-content">{easyProblemsCount}</span>
          </div>
          <div className="bg-base-100 border border-base-200 p-5 rounded-2xl shadow-sm flex flex-col">
            <span className="text-sm font-semibold text-warning">Medium</span>
            <span className="text-3xl font-bold mt-1 text-base-content">{mediumProblemsCount}</span>
          </div>
          <div className="bg-base-100 border border-base-200 p-5 rounded-2xl shadow-sm flex flex-col">
            <span className="text-sm font-semibold text-error">Hard</span>
            <span className="text-3xl font-bold mt-1 text-base-content">{hardProblemsCount}</span>
          </div>
        </div>
            ...
            >
              <div>
                <div className="flex items-start justify-between gap-4 mb-4">
                  ...
                  <span className={`px-3 py-1 text-xs font-bold uppercase tracking-wider rounded-lg border ${getDifficultyBadgeClass(problem.difficulty)} border-current bg-opacity-10`}>
                    {problem.difficulty}
                  </span>
                </div>
                
                <h2 className="text-xl font-bold text-base-content mb-1 group-hover:text-primary transition-colors">{problem.title}</h2>
                <p className="text-sm font-semibold text-base-content/50 uppercase tracking-widest mb-3">{problem.category}</p>
